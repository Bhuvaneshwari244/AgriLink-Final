...
import { getSampleQuestions } from "@/data/communityQuestionsTranslations";
...
export default function Community() {
  ...
  const [questions, setQuestions] = useState<Question[]>(() => {
    ...
    // Return translated sample questions based on current language
    return getSampleQuestions(lang);
  });
  ...
  // Update sample questions when language changes (only if using default samples)
  useEffect(() => {
    const saved = localStorage.getItem("agrilink-questions");
    if (!saved) {
      setQuestions(getSampleQuestions(lang));
    }
  }, [lang]);
