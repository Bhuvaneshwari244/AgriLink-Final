...
import { translateCropDetails } from "@/data/cropDetailsTranslations";
...
export default function CropLibrary() {
  ...
  if (selected) {
    ...
    const translatedDescription = translateCropDetails(selected.id, lang, 'description', selected.description);
    const translatedSeason = translateCropDetails(selected.id, lang, 'season', selected.season);
    const translatedSoilType = translateCropDetails(selected.id, lang, 'soilType', selected.soilType);
    const translatedIrrigation = translateCropDetails(selected.id, lang, 'irrigation', selected.irrigation);
    const translatedFertilizer = translateCropDetails(selected.id, lang, 'fertilizerSchedule', selected.fertilizerSchedule);
    const translatedPests = translateCropDetails(selected.id, lang, 'pests', selected.pests);
    const translatedBestPractices = translateCropDetails(selected.id, lang, 'bestPractices', selected.bestPractices);
    ...
    return (
      <PageTransition>
        <div className="container mx-auto px-4 py-6 max-w-3xl">
          ...
          <p className="text-secondary-foreground mb-6 leading-relaxed">{translatedDescription}</p>
          <div className="grid gap-4">
            {[
              { label: t.crops.season, value: translatedSeason },
              { label: t.crops.soil, value: translatedSoilType },
              { label: t.crops.irrigation, value: translatedIrrigation },
              { label: t.crops.fertilizer, value: translatedFertilizer },
            ].map((item, i) => (
            ...
            <div className="glass-card p-4">
              ...
              <div className="flex flex-wrap gap-2">{translatedPests.map((p: string) => <span key={p} className="bg-secondary text-secondary-foreground text-xs px-2.5 py-1 rounded-xl">{p}</span>)}</div>
            </div>
            <div className="glass-card p-4">
              <h3 className="text-sm font-semibold text-primary mb-2">{t.crops.bestPractices}</h3>
              <ul className="list-disc list-inside space-y-1 text-sm text-foreground">{translatedBestPractices.map((b: string) => <li key={b}>{b}</li>)}</ul>
            </div>
          </div>
